/*
 * 졸업작품 Visual Shell - 11월 14일까지의 변경사항.
 *
 * 소스  작성: {개인정보 제거 : fp024}
 *
 * 작성자소속: {개인정보 제거}
...
chtype volatile main_ipt_ch;    //getch 반환값 받음 /  2바이트 이상의 유니코드 문자도 받을 수 있음.
...
void execFunc(const char *filename); // Exec()류 함수들을 통해 실행파일 실행하기 위한 목적의 함수.

void execFilenameRev(const char *filename
        , const int name_num, char **exec_filename); // 실행파일명을 "./실행파일명" 으로 만들기.
...
void SelectedItemClipBoard(int selectItemCount[],  /* 선택된 아이템의 갯수 */
...
void PasteItemClipBoard(int selectItemCount[],  /* 선택된 아이템의 갯수 배열  0번은 이전 1번은 현재 */
                        char *absDIRstr,  /* Ctrl+V단축키가 눌려진 시점의 절대경로 */
                        CLIP_ENTITY **clip_items /* 아이템의 절대경로와 이름을 저장할 포인터 */
);


void DeleteItemClipBoard(int selectItemCount[],  /* 선택된 아이템의 갯수 배열  0번은 이전 1번은 현재 */
                         CLIP_ENTITY **clip_items   /* 아이템의 절대경로와 이름을 저장할 포인터 */
);


void Dlg_Chmod(WINDOW *dlg_win_chmod, int selectItemCount[], CLIP_ENTITY **clip_items);
        ...
        ,MENU **my_menu , const char *absDIRstr)
{
    ...
    if(strcmp(absDIRstr,"/")==0){   // 서브디렉토리에서는  "." 만
        filter_func=ddot_no_select;  // 루트디렉토리에서는 "." ".."을 제거하고 싶어서.
    }else{
    ...
    (*my_items)[*n_choices] = (ITEM *)NULL; // 결국은 my_items[]의 마지막에 NULL을 넣는다.
    *my_menu = new_menu( (ITEM **)(*my_items) );
    ...
    if(strcmp(absDIRstr,"/")!=0)    // 루트디렉터리가 아닐때 ".."이 선택되지 않도록 함.
    {
...
int main(void)
{
    ...
    WINDOW *entity_list_win, // 현재 디렉토리 안의 개체를 출력하는 메뉴리스트를 붙일 윈도우
    *shortcut_display_win, *entity_detail_display_win,
            *directory_display_win, *dlg_win_mkdir, *dlg_win_rename, *dlg_win_chmod;


    ITEM **my_items; // ITEM에 대한 포인터 배열의 첫번째 주소를 저장할 2중 포인터 변수.
    MENU *my_menu;
    int n_choices;  // 메뉴 아이템의 갯수를 저장할 변수 /
    //int min_x, min_y, max_x, max_y;   // getbegyx(), getmaxyx()와 같이 사용할 함수
    ...
    char usrIptName[100];  // 사용자가 입력한 파일명 또는 디렉토리 이름을 받을 공간.
    ...
    init_pair(8, COLOR_YELLOW, COLOR_BLUE); //  다이얼로그 박스 정보문장 및 버튼 색상


    getCurrDir(&absDIRstr, &maxpath); // 현재 디렉터리 얻어오기.
    // absDIRstr에 동적 메모리가 할당되므로 해제 꼭하기~~

    refresh(); // stdscr에 대한 refresh를 해주고 윈도우들을 wrefresh해줘야 제대로 나왔다.

    // ---------------- WINDOW 생성 및 설정 부 ------------------------------

    directory_display_win = newwin(3, COLS, 0, 0);      // 현재 디렉토리 표시 윈도우 생성.

    entity_list_win = newwin(LINES-7 , COLS , 3, 0 );   // 개체 목록 윈도우 생성.
    keypad(entity_list_win, TRUE);

    entity_detail_display_win = newwin(1, COLS, LINES-4, 0); // 상세 정보 표시 윈도우 생성.

    shortcut_display_win = newwin(3, COLS, LINES-3, 0); // 기능키 윈도우 생성.

    dlg_win_mkdir=newwin(8,COLS-20, LINES/2 - 8 , 10 ); // 디렉토리 생성 다이얼로그 박스.

    dlg_win_rename=newwin(8,COLS-20, LINES/2 - 8 , 10 );    // 이름 변경 다이얼로그 박스.

    dlg_win_chmod=newwin(10,COLS-20, LINES/2 - 8 , 10 );    // 권한 변경 다이얼로그 박스.
    ...
    EntityListWinDisplay(entity_list_win);  // 개체 목록 윈도우 표시. // initMenu와 연관됨.
    ...
    while(1)
    {
        ...
        if (main_ipt_ch == KEY_F(12)){
            break;
        }   // 27: ESC를 사용할 경우 다른 키입력에서도 종료되는 경우가 있어 F12로 변경
        ...
        switch(main_ipt_ch)
        {
            ...
            {   // Enter 내부 블럭 시작
                ...
                switch(pce_rtn_val)
                {
                    ...
                    case 2: // 실행파일은 실행되어지게 함.
                        ...
                        execFilenameRev(entity_name
                                , strlen(entity_name)+1, &exec_filename);
            ...
            case 0x03:  // Copy 명령
                copy1move2=1;
                SelectedItemClipBoard(&selectItemCount[0],  /* 선택된 아이템의 갯수 */
                ...
                mvwaddch(shortcut_display_win, 1 , COLS-2, 'C'
            ...
            case 0x0f:  // Move 명령
                ...
                mvwaddch(shortcut_display_win, 1 , COLS-2, 'O'
                                                           | A_BOLD | A_BLINK | COLOR_PAIR(4) );
            ...
            case 0x04:  // Delete 명령
                ...
                mvwaddch(shortcut_display_win, 1 , COLS-2, 'D'
                                                           | A_BOLD | A_BLINK | COLOR_PAIR(4) );
            ...
            case 0x0b:  // Mkdir 명령
                ...
                if(usrIptName[0]!='\0')  // 취소 버튼을 눌렀다면 '\0'이므로 메뉴재생성필요없음
                {
                    // 디렉토리가 바뀌는 이벤트와 마찬가지로 메뉴 재생성.
                    freeDynamicMem(&my_menu, &my_items, &d_ptrArray
                            , &rtn_entity_size_chr
                ...
                update_panels();
                mvwaddch(shortcut_display_win, 1 , COLS-2, 'K'
                                                           | A_BOLD | A_BLINK | COLOR_PAIR(4) );
                break;
            ...
            case 0x08:  // Chmod 명령
                SelectedItemClipBoard(&selectItemCount[0],  /* 선택된 아이템의 갯수 */
                                      ...
                                      &clip_items,    /* 아이템의 절대경로와 이름을 저장할 포인터 */
                                      &bakAbsDIRstr
                );  // 다중 개체 권한 변경이 가능해야하므로 이 함수 호출 후
                //
                ...
                mvwaddch(shortcut_display_win, 1 , COLS-2, 'H'
                                                           | A_BOLD | A_BLINK | COLOR_PAIR(4) );
                break;
            ...
            case 0x16:  // Paste 명령
                ...
                mvwaddch(shortcut_display_win, 1 , COLS-2, 'V'
                                                           | A_BOLD | A_BLINK | COLOR_PAIR(4) );
                PasteItemClipBoard(&selectItemCount[0],
        ...
        // 파일 정보 [이름] - [크기] - [수정날짜] - [소유자.그룹]  - [권한] 을 표시해야할 부분.
        {   // 내부 블럭 시작:/
            ITEM *curItem;
    ...
    delwin(entity_list_win); // WINDOW를 메모리에서 해제 그러나 이미 출력된 이미지가 제거되지는 않음
...
int print_current_entity(const char *entity_name, const int pce_mode, WINDOW *entity_detail_display_win)
{
    ...
    const char file_perm_templet[10]="rwxrwxrwx"; // 파일권한 문자들의 틀을 위한 상수
    char view_perm[10];
    ...
    if(lstat(entity_name, &statbuf)==-1)
    {
        ...
        endwin();
        fprintf(stderr, "(오류번호: %d) - \"%s\" 파일명을 인수로 lstat함수를 사용할 때"
                        "오류가 발생했습니다.\n", errno, entity_name);
        perror(" ");
        exit(-9);
    }
    ...
    if( S_ISREG(statbuf.st_mode)==1 ) { //일반 파일
        ...
        file_attribute='b';
    }else if( S_ISCHR(statbuf.st_mode) ) {  // character special file; 문자 특수 파일
        file_attribute='c';
    ...
    for(i=0; i<9; i++)
    {
        if(statbuf.st_mode & octperm[i]) { // 비트별 AND를 사용하여 허가가 설정되었는지 테스트
    ...
    wprintw(entity_detail_display_win, "[%d Byte] [%s.%s] [%c%s] [%04d/%02d/%02d-%02d:%02d]"
...
{   // 개체의 이름을 입력 받으면 크기를 계산하는 함수.
    ...
    if(lstat(entity_name, &statbuf)==-1)
    {
        if(errno==75) // lstat()가 2GB이상의 크기의 파일을 읽을 때 발생하는 오류번호
        {       // 해결책 못찾음.
...
void execFunc(const char *filename) // 실행파일 선택 상태에서 엔터 이벤트가 일어났을 때 수행하는 함수.
{

    int status;
    pid_t rtn_fork;

    //sigset_t mask, oldmask; // 새로 값을 지정할 마스크와 이전 마스크를 저장할 변수.
    //          // 교제2) 342쪽
    ...
    rtn_fork=fork();
    switch(rtn_fork)    // 하나의 자식만 만듦 .. 단순 엔터에서 추가 인수넣는 것을 고려하지 않음.
    {
        case -1:    // fork오류
    ...
    write(STDOUT_FILENO, filename, strlen(filename)+1); // filename은 char* 이니 sizeof를 사용하면X
    write(STDOUT_FILENO, pause_msg, sizeof(pause_msg));
    // printf()출력이 바로 작동하지 않아 write()사용..

    getchar();  // 자식프로그램 종료후 키보드 입력 대기.

    act_exec=oldact;    // 자식 프로세스가 실행되는 동안 Ctrl + C 가 입력 되었을 때
...
void execFilenameRev(const char *filename, const int name_num, char **exec_filename)
{
...
void ShortCutWinDisplay(WINDOW *shortcut_display_win) // 단축키를 표시할 윈도우
{
    ...
    // Move의 단축키를 m으로 하고 싶었는데, ENTER입력과 중복되서 일단은 o로 바꿔둠.
    ...
    for(i=1; i<9 ;i++)
    {
        ...
        // 작품 판넬에 낸 것처럼 한글의 좌표위치를 일일이 지정해주는 것은 문제가 있다고 판단하여
        // 영문으로 바꿔봄.
...
//다중선택된 개체들의 전체 경로, 파일명|디렉토리명을 메모리에 보관하는 함수 구현

void SelectedItemClipBoard(int selectItemCount[],  /* 선택된 아이템의 갯수배열  0번은 이전 1번은 현재 */
...
)
{
    ...
    {
        for(i=0; i < selectItemCount[0]; i++) // 과거의 갯수가 저장되있는 것을 메모리 해제.
        {
    ...
    {
        endwin();
        perror("malloc()오류 - 위치: SelectedItemClipBoard() -"
    ...
    if( (*clip_items = (CLIP_ENTITY *)malloc(  sizeof(CLIP_ENTITY) * (selectItemCount[1]) ))==NULL )
    {
        endwin();
        perror("malloc()오류 - 위치: SelectedItemClipBoard() -"
    ...
    for(i=0; i < AllItemNum; i++) // 전체 아이템의 갯수 만큼 반복
    {
        if( item_value( (*my_items)[i] ) == TRUE )  // 아이템이 선택되었는지 검사.
        {
            ...
            if(((*clip_items)[j].name = (char *)malloc( (etc_num+1) * sizeof(char) ))==NULL)
            {
                endwin();
                perror("malloc()오류 - 위치: SelectedItemClipBoard() -"
    ...
    if( ((*clip_items)[0].abspath=(char *)malloc( sizeof(char)*etc_num))== NULL )
    {
        endwin();
        perror("malloc()오류 - 위치: SelectedItemClipBoard() -"
    ...
    selectItemCount[0]=selectItemCount[1]; // 메모리할당이 끝난후 현재의 갯수를 과거에 할당.
...
)
{
    ...
    if(*clip_items==NULL || selectItemCount[1]==0)  // 현재 선택된 아이탬이 0이거나
    ...
    for(i=0; i<selectItemCount[1]; i++ )
    {
        ...
        if( (src_full_path_name = (char *)malloc(
                sizeof(char) * (strlen((*clip_items)[i].name)
                                +strlen((*clip_items)[i].abspath) + 2  )
        ))==NULL )
        {
            endwin();
            perror("malloc()오류 - 위치: PasteItemClipBoard() -"
                   "원본파일 전체경로 문자열  포인터에 동적메모리 할당하는 부분.");
            exit(-24);
        } //if end

        // '\0'과 파일과 절대경로 사이에 '/'하나를 넣기 위해서. + 2
        if( (des_full_path_name = (char *)malloc(
                sizeof(char) * (strlen((*clip_items)[i].name)
                                +strlen(absDIRstr) + 2  )
        ))==NULL )
        {
        ...
        sprintf(src_full_path_name, "%s/%s"
                , (*clip_items)[i].abspath
                , (*clip_items)[i].name
        );

        sprintf(des_full_path_name, "%s/%s"
                , absDIRstr
                , (*clip_items)[i].name
        );
        ...
        if( (desfd=open(des_full_path_name
                ,  O_WRONLY   /* 쓰기전용으로 연다.  */
                   ...
                   /* | O_TRUNC*/ /* 이미 존재하는 파일을 [R/W]연다면 크기를 0으로 만듦. */
        ...
        )) == -1 )
        {
            ...
            continue;
        }   /* 목적지를 O_TRUNC로 해두면 한경로에서 선택후 복사명령으로 클립보드에 붙힌후
               경로를 바꿨다가 원래자리로 돌아온후 붙혀넣으면 복사할 파일의 크기를
               0으로 만들어버리는 문제가 있다.
              */
        ...
        {
            if( write(desfd, buffer, nread) < nread )
            {   // write()로 쓴 바이트 수가 nread와 같아야 정상인데,
                // 다르다면 오류...
                close(srcfd);
                close(desfd);
                exit(-26);
...
)
{
    ...
    for(i=0; i<selectItemCount[1]; i++ )
    {
        ...
        {
            endwin();
            perror("malloc()오류 - 위치: DeleteItemClipBoard() -"
        ...
        sprintf(full_path_name, "%s/%s"
                , (*clip_items)[i].abspath
                , (*clip_items)[i].name
        );

        // stdio.h에서 제공하는 C라이브러리 함수. // Dialog박스를 출력해야하는데.. 아직안되서.
        remove(full_path_name);   // 디렉토리 및 파일 삭제를 할 수 있으므로 좀 위험한 것 같다.
        free(full_path_name);
        full_path_name=(char *)NULL;
    }
    ...
    for(i=0; i < selectItemCount[1]; i++) // 과거의 갯수가 저장되있는 것을 메모리 해제
...
int ddot_no_select(const struct dirent *entry)   // "." ".." 제외 목적의 filter함수.,
{
...
void Dlg_Rename(WINDOW *dlg_win_rename, const char *prevName, char *newName)
{
    ...
    // 제외 문자를 둬보고 싶어서 백스페이스키와 영문 숫자 기타 특수문자만 허용하게 해봄.
    // 한글 이름 입력이 안 되는 문제가 있음.
    ipt=0, i=0;
...
void Dlg_Chmod(WINDOW *dlg_win_chmod, int selectItemCount[], CLIP_ENTITY **clip_items)
{
    ...
    {
        return;     // SelectedItemClipBoard()가 제대로 실행되지 않았다면 함수 바로 종료.
    }
    ...
    mvwprintw(dlg_win_chmod, 2, 22, "%s 포함 %d개 개체"
            , (*clip_items)[0].name, selectItemCount[1]);
    ...
    free( (char *)((*clip_items)[0].abspath) ); // 절대경로 정보를 갖고있지만 이 함수에선 사용안함.
    (*clip_items)[0].abspath=(char *)NULL;
